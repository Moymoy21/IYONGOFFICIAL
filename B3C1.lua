...
createCustomButton("Section 4", "Auto C4 Bomb Part 1", "Fast-tweens and plants all 3 C4 bombs for Part 1", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    local TweenService = game:GetService("TweenService")
    ...
    -- Helper function to perform fast linear tweens
    local function fastTweenTo(targetCFrame, speed)
        local distance = (humanoidRootPart.Position - targetCFrame.Position).Magnitude
        local duration = distance / (speed or 250)
        ...
        tween:Play()
        tween.Completed:Wait()
        task.wait(0.1) -- Mobile stability refresh frame
    end

    -- Verify Objective2 folder structure
    ...
    -- List of your 3 C4 coordinates for Part 1
    local bombLocations = {
    ...
    -- Dynamic selector to grasp the closest available bomb prompt
    local function getClosestBombPrompt()
        ...
        for _, child in ipairs(c4Folder:GetChildren()) do
            ...
            if prompt then
                -- Fallback to search child sub-models if embedded inside a basepart model layer
    ...
    for index, targetCFrame in ipairs(bombLocations) do
        notify("Progress", "Tweening to C4 Bomb [" .. index .. "/3]...", 1)
        fastTweenTo(targetCFrame, 250)
        task.wait(0.3) -- Sync delay for mobile cache processing

        local targetPrompt = getClosestBombPrompt()
        if targetPrompt then
            safeFirePrompt(targetPrompt)
            notify("Success", "C4 Bomb #" .. index .. " planted!", 1.5)
            task.wait(0.8) -- Delay to let planting register before next glide
        else
            notify("Warning", "Prompt not found at spot #" .. index, 1.5)
        end
    end

    notify("Complete", "Part 1 C4 setup finished!", 4)
end)

createCustomButton("Section 4", "Lab Quest Auto-Farm", "Automates ID Card, Gate, Computer, and Laptop Password Submission", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
createCustomButton("Section 4", "Auto C4 Bomb Part 2", "Triggers area transition and plants all 6 C4 bombs", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    local TweenService = game:GetService("TweenService")
    ...
    -- =========================================================================
    -- INITIAL AREA TRANSITION TRIGGER
    -- =========================================================================
    notify("Transition", "Gliding to initial trigger zone...", 2)
    local transitionZone = CFrame.new(-4146.615, 106.991, 2298.599)
    fastTweenTo(transitionZone, 250)
    task.wait(0.5) -- Wait buffer for area context rendering

    -- List of your 6 C4 coordinates for Part 2
    local bombLocations = {
    ...
    notify("C4 Farm Active", "Gliding to plant 6 C4 charges...", 3)
    ...
    notify("Complete", "Part 2 C4 sequence fully executed!", 4)
end)
...
local renderConnections = {} -- Imbakan ng loops para sa frame tracking
_G.HogoRadarActive = false -- Variable para sa buhay ng radar thread

createToggle("Section 4", "HogoGuntai Box & Name ESP", "Highlights HogoGuntai with solid filled proxy box and activates 25-stud radar", function(state)
    -- Kumuha o gumawa ng lalagyan sa CoreGui para sa mga visual elements
    local espContainer = CoreGui:FindFirstChild("IOHUB_HogoProxyContainer")
    if not espContainer then
        espContainer = Instance.new("Folder")
        espContainer.Name = "IOHUB_HogoProxyContainer"
        espContainer.Parent = CoreGui
    end

    -- Function para sa Pangalan (Name Tag)
    local function applyHogoNameTag(part)
        if not part or not part:IsA("BasePart") then return end
        if espContainer:FindFirstChild("Tag_Hogo_Single") then return end
        
        local billboard = Instance.new("BillboardGui")
        billboard.Name = "Tag_Hogo_Single"
        billboard.Size = UDim2.new(0, 200, 0, 50)
        billboard.AlwaysOnTop = true -- Tagos pader sa mobile screen
        billboard.ExtentsOffset = Vector3.new(0, 4.5, 0)
        billboard.Adornee = part
    ...
    -- PROXY FILLED ENGINE FUNCTION (Bypasses monster shaders/transparency)
    local function createProxyFilledESP(monsterPart)
        if not monsterPart or not monsterPart:IsA("BasePart") then return end
        
        local proxyName = "Proxy_" .. monsterPart.Name
        if espContainer:FindFirstChild(proxyName) then return end
        
        -- Gumawa ng independent proxy brick para sa solid coloring supremacy
        local proxyPart = Instance.new("Part")
        proxyPart.Name = proxyName
        proxyPart.Size = Vector3.new(5, 7, 5) -- Force solid box size
        proxyPart.Transparency = 1 
        proxyPart.Anchored = true
        proxyPart.CanCollide = false
        proxyPart.Parent = espContainer
        
        -- Ikabit ang Adornment Box sa proxy part para laging may kulay ang loob
        local box = Instance.new("BoxHandleAdornment")
        box.Name = "FilledShell"
        box.Size = proxyPart.Size
        box.AlwaysOnTop = true
        box.ZIndex = 15
        box.Color3 = Color3.fromRGB(255, 140, 0) -- Solid Warning Orange
        box.Transparency = 0.5 -- 50% solid translucent filled color
        box.Adornee = proxyPart
        box.Parent = proxyPart
        
        if monsterPart.Name == "HumanoidRootPart" then
            applyHogoNameTag(proxyPart)
        end
        
        -- Piliting sumunod ang proxy brick sa monster frame-by-frame
        if renderConnections[monsterPart] then renderConnections[monsterPart]:Disconnect() end
        
        renderConnections[monsterPart] = RunService.RenderStepped:Connect(function()
            if monsterPart and monsterPart.Parent and proxyPart then
                proxyPart.CFrame = monsterPart.CFrame
            else
                if renderConnections[monsterPart] then
                    renderConnections[monsterPart]:Disconnect()
                    renderConnections[monsterPart] = nil
                end
                if proxyPart then proxyPart:Destroy() end
            end
        end)
    end

    -- Function para mag-scan ng parts
    local function scanHogoParts(object)
        if not object or not object.Parent then return end
        if object.Parent.Name == "HogoGuntai" then
            if object.Name == "HumanoidRootPart" or object.Name == "Hitbox" then
                createProxyFilledESP(object)
            end
        end
    end
    ...
    if state then
        -- ===== [TOGGLE ON] =====
        -- 1. I-scan agad ang workspace kung naka-spawn na si HogoGuntai
        ...
        if monsterFolder then
            ...
            if hitbox then createProxyFilledESP(hitbox) end
            if hrp then createProxyFilledESP(hrp) end
        end

        -- 2. Makinig sa buong Workspace gamit ang DescendantAdded para sa live tracking
        ...
        -- 3. INTEGRATED EMERGENCY RADAR DETECTOR (25-Stud Radius SafeZone Window)
        ...
        notify("IOHUB ACTIVE", "📦 Proxy Filled ESP & 25-Stud Radar: ON", 3)
    else
        -- ===== [TOGGLE OFF] =====
        -- 1. Putulin ang live descendant listener
        ...
        -- 3. Putulin ang lahat ng active frame tracking connections at linisin ang proxy parts
        for part, connection in pairs(renderConnections) do
        ...
        notify("IOHUB STATUS", "⚪ HogoGuntai ESP & Radar: OFF", 2)
    end
end)
