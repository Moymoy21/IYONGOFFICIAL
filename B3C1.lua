...
-- ====================================================================
-- AUTO C4 BOMB PART 2 BUTTON WRAPPER (MAX SPEED TWEEN ENGINE UPGRADE)
-- ====================================================================
createCustomButton("Section 4", "Auto C4 Bomb Part 2", "Fast tweens and plants all 6 C4 charges for Part 2", function()
    ...
    local RunService = game:GetService("RunService")
    local StarterGui = game:GetService("StarterGui")
    local TweenService = game:GetService("TweenService") -- Added safely within button scope
    ...
    -- Integrated high-speed linear tween implementation (Speed 250) [INDEX]
    local function fastTweenTo(targetCFrame)
        local distance = (humanoidRootPart.Position - targetCFrame.Position).Magnitude
        local duration = distance / 250 -- Fixed 250 studs per second velocity scale [INDEX]
        
        pcall(function()
            humanoid:ChangeState(Enum.HumanoidStateType.Running) -- Force wide state hitbox [INDEX]
        end)
        ...
        tween:Play()
        tween.Completed:Wait() -- Force sync block execution until arrival
        task.wait(0.1) -- Small physics stabilization delay
    end

    -- Anti-Push Engine para sa Part 2 (Kept exactly as requested) [INDEX]
    local function safeFirePromptWithLock(prompt, targetCFrame)
        if not prompt then return false end
        ...
        lockConnection = RunService.Heartbeat:Connect(function()
            if positionLockActive and humanoidRootPart then
                humanoidRootPart.CFrame = targetCFrame
                humanoidRootPart.Velocity = Vector3.new(0, 0, 0) -- Freezes all back-push forces [INDEX]
            end
        end)
        
        fireproximityprompt(prompt)
        task.wait(0.6) -- Sealed verification time window
    ...
    -- 1. INITIAL AREA TRANSITION TWEEN (Upgraded to linear glide) [INDEX]
    notify("Transition", "Moving to initial trigger zone...", 2)
    local transitionZone = CFrame.new(-4146.615, 106.991, 2298.599)
    
    fastTweenTo(transitionZone) -- Glides at max speed to trigger the cinematic touchpads [INDEX]
    task.wait(0.6) -- Wait buffer for area context rendering [INDEX]

    -- List of your 6 C4 coordinates for Part 2 [INDEX]
    local bombLocations = {
    ...
    local getClosestBombPrompt = function()
        local closestPrompt = nil
        local shortestDistance = math.huge
        for _, child in ipairs(c4Folder:GetChildren()) do
            local prompt = child:FindFirstChild("ProximityPrompt") or child:FindFirstChildWhichIsA("ProximityPrompt", true)
    ...
    -- Loop through the 6 spots sequentially using the fast tween framework [INDEX]
    for index, targetCFrame in ipairs(bombLocations) do
        notify("Progress", "Tweening to C4 Bomb [" .. index .. "/6]...", 1)
        
        fastTweenTo(targetCFrame) [INDEX]
        task.wait(0.3) -- Mobile map streaming sync safety wait
...
createCustomButton("Section 4", "Cleanse Room Farm", "Automates all valves and shape terminals sequentially by scanning folders", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
