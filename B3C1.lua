...
createCustomButton("Section 3", "Restore Power Max-Speed", "Max-speed circuit bypass with cinematic destination tween", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    ...
    -- =========================================================================
    -- CINEMATIC TWEEN TO FINAL DESTINATION (WALL TOUCH/HIT TRIGGER ENGINED)
    -- =========================================================================
    notify("Farm Finished", "All circuits restored! Gliding to trigger cinematic...", 3)
    task.wait(0.5)
    
    local finalDestinationCFrame = CFrame.new(-3389.176, 8.000, 807.249)
    
    -- Calculate the exact duration based on a human-like walking speed (12 studs per second)
    -- This ensures your character physically passes through and hits wall checkpoints/triggers
    local currentPosition = humanoidRootPart.Position
    local distance = (currentPosition - finalDestinationCFrame.Position).Magnitude
    local walkSpeed = 12 
    local tweenDuration = distance / walkSpeed

    -- Force Humanoid into running state so hitboxes are wide open to register wall touches
    pcall(function()
        humanoid:ChangeState(Enum.HumanoidStateType.Running)
    end)

    local tweenInfo = TweenInfo.new(tweenDuration, Enum.EasingStyle.Linear)
    local cinematicTween = TweenService:Create(humanoidRootPart, tweenInfo, {CFrame = finalDestinationCFrame})
    
    cinematicTween:Play()
    cinematicTween.Completed:Wait() -- Wait until the character physically hits the final target zone
    
    notify("Success", "Arrived at zone! Cinematic should be triggered.", 4)
end)
