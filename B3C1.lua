...
createCustomButton("Section 3", "Restore Power Max-Speed", "Max-speed circuit bypass with final teleport location", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    ...
    -- Helper function para sa ligtas na pag-fire ng ProximityPrompt
    local function safeFirePrompt(prompt)
        if not findFirstChild then
            prompt.RequiresLineOfSight = false
            prompt.MaxActivationDistance = 50
            task.wait(0.05) -- Pabilisang delay bago itulak ang prompt
            fireproximityprompt(prompt)
        end
        return true
    end
    ...
    -- Kunin ang eksaktong Cobalt Module Event base sa iyong log
    ...
    if not CobaltEvent then
        notify("Critical Error", "Hindi mahanap ang Cobalt Packet Route!", 4)
        return
    end
    ...
    -- Helper function para hanapin ang PromptPart at ProximityPrompt na pinakamalapit sa player pagkalapag
    ...
    for index, targetCFrame in ipairs(circuitPositions) do
        ...
        humanoidRootPart.CFrame = targetCFrame
        task.wait(0.4) -- Katamtamang delay para sa mobile physics rendering

        -- Hanapin ang prompt at model na pinakamalapit sa spot na ito
        local prompt, circuitModel = getClosestCircuitPrompt()

        if prompt and circuitModel then
            -- 1. Buksan ang interaction link sa server
            safeFirePrompt(prompt)
            task.wait(0.2) 

            -- 2. MAXIMUM SPEED KICK-BYPASS LOOP (0.1 seconds interval)
            for round = 1, 3 do
                CobaltEvent:FireServer("Section3/CircuitRoundComplete", circuitModel)
                task.wait(0.1) -- Ginawang 0.1s para sa instant-kill completion rate
            end
            
            task.wait(0.4) -- Pinabilis na delay bago lumipat sa susunod na machine
        else
            notify("Warning", "Circuit Prompt not Found in Spot #" .. index, 1)
        end
    end

    -- =========================================================================
    -- FINAL LOCATION TELEPORTATION (BAGONG DAGDAG)
    -- =========================================================================
    notify("Finished", "All Circuis Done", 2)
    task.wait(0.5)
    
    local finalDestinationCFrame = CFrame.new(-3389.176, 8.000, 807.249)
    ...
    humanoidRootPart.CFrame = finalDestinationCFrame
    notify("Mission Complete", "Trigger Cutscene", 4)
end)
