...
createCustomButton("Section 1", "Skip Enzukai", "Teleport to White Door", function()
...
CreateCustomButton("Section 1", "Civilian Interact", "Teleport to Civilian and Interact", function()
    ...
    -- List of all 6 civilian data (CFrame and their specific path/prompt if needed)
    local civilians = {
        { cf = CFrame.new(82.2101974, 7.01562452, 229.03598, 0.999974549, 1.60358249e-09, -0.00713643711, -1.60001368e-09, 1, 5.05793407e-10, 0.00713643711, -4.94362107e-10, 0.999974549) },
        { cf = CFrame.new(29.5840073, 7.05437136, -87.9759293, 0.999974549, 7.98299926e-10, -0.00713643711, -7.60655594e-10, 1, 5.27766053e-09, 0.00713643711, -5.27209787e-09, 0.999974549) },
        { cf = CFrame.new(-182.772629, 7.01562452, 73.8156357, 0.0309865493, 1.37417135e-08, 0.999519825, -2.71963851e-09, 1, -1.36640024e-08, -0.999519825, -2.29493224e-09, 0.0309865493) },
        { cf = CFrame.new(225.873566, 7.03801394, -84.7873001, 0.999954641, 6.97570335e-10, 0.00952569582, -9.66604463e-10, 1, 2.82383912e-08, -0.00952569582, -2.82463173e-08, 0.999954641) },
        { cf = CFrame.new(-3.58142662, 7.01562452, -176.386215, -0.992575169, -1.97492529e-08, -0.121632636, -1.93827052e-08, 1, -4.19672697e-09, 0.121632636, -1.80799731e-09, -0.992575169) },
        { cf = CFrame.new(80.6713104, 7.0411253, 149.687668, -0.997461557, 1.115201e-08, -0.0712069646, 1.04766196e-08, 1, 9.85837101e-09, 0.0712069646, 9.08733799e-09, -0.997461557) }
    }

    -- Loop through each civilian
    for i, data in ipairs(civilians) do
        -- 1. Teleport muna sa civilian
        humanoidRootPart.CFrame = data.cf
        task.wait(0.2) -- Konting hintay para mag-load yung position

        -- 2. Kunin yung prompt gamit ang eksaktong path mo
        -- Note: Kung hiwalay ng pangalan bawat civilian (e.g., Civilian1, Civilian2), 
        -- pwede nating palitan yung "Civilian" ng `data.civName` kung sakali.
        local prompt = Workspace:FindFirstChild("Section1") 
            and Workspace.Section1:FindFirstChild("DeadCivilians") 
            and Workspace.Section1.DeadCivilians:FindFirstChild("Civilian") 
            and Workspace.Section1.DeadCivilians.Civilian:FindFirstChild("HumanoidRootPart") 
            and Workspace.Section1.DeadCivilians.Civilian.HumanoidRootPart:FindFirstChild("ProximityPrompt")

        -- 3. Fire nung prompt kung nahanap
        if prompt then
            fireproximityprompt(prompt)
        end

        -- 4. Wait 2 seconds bago lumipat sa sunod
        task.wait(2)
    end
end)
...
CreateCustomButton("Section 1", "Hideo Interact", "Teleport to Hideo and Interact", function()
    ...
    -- 1. Teleport muna sa Hideo location gamit ang direct CFrame
    humanoidRootPart.CFrame = CFrame.new(107.692, 7.064, 269.127)
    task.wait(0.2) -- Konting hintay para mag-load yung position

    -- 2. Kunin at i-fire yung ProximityPrompt gamit ang path
    local prompt = Workspace:FindFirstChild("Section1") 
        and Workspace.Section1:FindFirstChild("HideoScene") 
        and Workspace.Section1.HideoScene:FindFirstChild("Hideo2") 
        and Workspace.Section1.HideoScene.Hideo2:FindFirstChild("HumanoidRootPart") 
        and Workspace.Section1.HideoScene.Hideo2.HumanoidRootPart:FindFirstChild("ProximityPrompt")
...
CreateCustomButton("Section 1", "Unlock Gate & School Door", "Get Key and Unlock The Gate and School Door", function()
    ...
    -- 1. Teleport to BoltCutter location and fire prompt to get key
    ...
    if boltPrompt then
        fireproximityprompt(boltPrompt)
    end
    task.wait(0.5)

    -- 2. Teleport to Gate Lock and fire prompt 4 times
    ...
    if gatePrompt then
        for i = 1, 4 do
            fireproximityprompt(gatePrompt)
            task.wait(0.2)
        end
    end
    task.wait(0.5)

    -- 3. Teleport to School Door and open it
