...
if not spidersFolder then
    ...
    task.spawn(function()
        while _G.SpiderKillLoopActive do
            ...
            -- INTERLOCK ESCAPE GATEWAY: Breaks Phase 1 once the uncompleted targets count drops to 0 [INDEX]
            ...
            if targetSpiderModel and targetCFrame and _G.SpiderKillLoopActive then
                ...
                -- Secure target position coordinates 4 studs below the spider hitbox grid mesh [INDEX]
                local safeSnapCFrame = targetCFrame * CFrame.new(0, -4, 0)
                humanoidRootPart.CFrame = safeSnapCFrame
                task.wait(0.15) 

                -- HEARTBEAT GLUE LAYER: Locks your root part tightly at point-blank range [INDEX]
                local isLockingActive = true
                local lockConnection
                lockConnection = RunService.Heartbeat:Connect(function()
                ...
                -- TIMED 2-SECOND HYPER-SPAM COOLDOWN WINDOW: Fires multiple damage packet injection waves [INDEX]
                ...
                while _G.SpiderKillLoopActive and (os.clock() - startTime < 2.0) do
                    ...
                    task.wait(0.1) -- Rapid fire rate interval delay [INDEX]
                end

                -- Disconnect the heartbeat frame physics weld [INDEX]
                isLockingActive = false
                if lockConnection then lockConnection:Disconnect() end
                
                -- Permanently blacklist this unique model group memory address [INDEX]
        ...
        -- =========================================================================
        -- PHASE 2: AUTOMATIC FINAL TELEPORT & RELIABLE TIMED EXIT PROMPT CHECK [INDEX]
        -- =========================================================================
        if _G.SpiderKillLoopActive then
            ...
            -- Teleport directly to your requested school exit checkpoint coordinate node [INDEX]
            local exitDoorCFrame = CFrame.new(174.953, 7.586, 513.158)
            humanoidRootPart.CFrame = exitDoorCFrame
            task.wait(0.3) -- Basic coordinate settle gap [INDEX]

            -- =========================================================================
            -- TIMED DELAY UPGRADE: Forced 2-second delay to settle server components [INDEX]
            -- =========================================================================
            for secondsLeft = 2, 1, -1 do
            ...
            -- Recursive lookup loop structure using active timeout verification gates [INDEX]
            ...
            while not exitPrompt and scanAttempts < 30 and _G.SpiderKillLoopActive do
                ...
                -- Dynamic tree evaluation checking your verified path string [INDEX]
                ...
                -- Fallback Tree Scanning check running concurrently in case indexing fails [INDEX]
                ...
                if not exitPrompt then 
                    task.wait(0.1) -- Refresh cycle frequency [INDEX]
                end
            end

            -- Execute the prompt interaction footprint sequence point-blank [INDEX]
            if exitPrompt then

safeFirePrompt(exitPrompt) [INDEX]
