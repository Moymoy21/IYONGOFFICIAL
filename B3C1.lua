...
-- ====================================================================
-- INDEPENDENT COBALT SHAPE TERMINAL BYPASS BUTTON
-- ====================================================================
createCustomButton("Section 4", "Auto Terminals Only", "Bypasses all shape terminals instantly and moves to Contain", function()
    ...
    if shapeTerminalsFolder then
        ...
        notify("Terminals Scan", "Discovered " .. tostring(#terminalsList) .. " Terminal blocks to clear.", 2.5)
        ...
        for index, terminalModel in ipairs(terminalsList) do
            ...
            if terminalModel:IsA("Model") then
                ...
                if terminalPrompt and promptPart then
                    notify("Terminal Farm", "Moving to Terminal spot #" .. index, 1.5)
                    
                    pcall(function() humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end)
                    task.wait(0.01)
                    
                    -- Aligns completely to current unique model group pivot matrix coordinates
                    ...
                    -- Execute exact Cobalt 2-round puzzle bypass sequences per unit frame
                    for round = 1, 2 do
                        if not _G.CleanseRoomActive then break end
                    ...
                    task.wait(0.5) -- Post-terminal system synchronization spacing buffer
                end
            end
        end
    end
    ...
    -- =========================================================================
    if _G.CleanseRoomActive then
        notify("Terminals Clear", "Teleporting to Contain location...", 2.5)
        task.wait(0.5)
...
createToggle("Section 4", "HogoGuntai Box & Name ESP", "Highlights HogoGuntai with synchronized wireframe and activates 25-stud radar", function(state)
    -- Kumuha o gumawa ng lalagyan sa CoreGui para sa mga visual elements
...
-- ====================================================================
-- SECTION 5 AUTO POWER SWITCH & WIREBOXES AUTOMATION BUTTON
-- ====================================================================
createCustomButton("Section 5", "Auto Switch & Wireboxes", "Automates Main Switch and wireboxes with auto UI release screen clear", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    ...
    -- Global variable thread configuration to allow safe interlocking cancellations
    _G.Section5Active = false
    task.wait(0.15)
    _G.Section5Active = true
    _G.ClearedWireBoxes = {} -- Blacklist tracking array for completed nodes
    ...
    -- Line of Sight and Safety Overrider
    local function safeFirePrompt(prompt)
        if not prompt then return false end
        local originalRequiresLineOfSight = prompt.RequiresLineOfSight
        local originalMaxDistance = prompt.MaxActivationDistance
    ...
    -- Core Section 5 Folder Path Validation
    local mainObjectiveFolder = Workspace:FindFirstChild("Section5")
        and Workspace.Section5:FindFirstChild("MainObjective")

    if not mainObjectiveFolder then
        notify("Critical Error", "Section 5 MainObjective folder not found!", 4)
        return
    end

    local powerSwitchModel = mainObjectiveFolder:FindFirstChild("PowerSwitch")
    local boxesFolder = mainObjectiveFolder:FindFirstChild("Boxes")

    -- =========================================================================
    -- STEP 1: TELEPORT TO MAIN SWITCH AND FIRE FOR THE FIRST TIME
    -- =========================================================================
    if _G.Section5Active and powerSwitchModel then
        notify("Step 1/3", "Moving to Main Power Switch...", 2)
        ...
        if switchPrompt then
            ...
            notify("Success", "Main Switch initialized!", 1.5)
        else
            notify("Warning", "Main Switch prompt not found. Proceeding...", 2)
        end
        task.wait(1)
    end

    -- =========================================================================
    -- STEP 2: DYNAMIC WIREBOX COBALT BYPASS (WITH AUTO UI-RELEASE FIXED)
    -- =========================================================================
    if _G.Section5Active and boxesFolder then
        notify("Step 2/3", "Initializing Wirebox Injection & Screen Clear...", 2)
        task.wait(0.2)

        while _G.Section5Active do
            local currentBoxesList = boxesFolder:GetChildren()
            local targetBox = nil
            local remainingWireBoxes = 0

            -- Dynamic Folder Scanning prevents errors if models are deleted or indices rearrange
            for _, obj in ipairs(currentBoxesList) do
                if (obj.Name == "WireBox" or obj:IsA("Model")) and not _G.ClearedWireBoxes[obj] then
                    remainingWireBoxes = remainingWireBoxes + 1
                    if not targetBox then
                        targetBox = obj -- Lock onto the first uncleaned wirebox instance found
                    end
                end
            end

            -- Break loop if there are absolutely no wireboxes left to process
            if remainingWireBoxes == 0 or not targetBox or not _G.Section5Active then
                notify("Wireboxes Clear", "All active wireboxes successfully bypassed!", 2)
                break
            end

            notify("Wirebox Target", "Processing network payload for " .. targetBox.Name, 1.5)

            -- Teleport directly onto current target pivot for positioning sync footprint verification
            pcall(function() humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end)
            task.wait(0.01)
            humanoidRootPart.CFrame = targetBox:GetPivot()
            task.wait(0.4)

            -- Fire ProximityPrompt to visually load the element context channels
            local innerPrompt = targetBox:FindFirstChild("PromptPart") and targetBox.PromptPart:FindFirstChildOfClass("ProximityPrompt")
                or targetBox:FindFirstChildWhichIsA("ProximityPrompt", true)
            if innerPrompt then
                safeFirePrompt(innerPrompt)
                task.wait(0.15)
            end

            -- A. EXACT COBALT COMPLETED OVERRIDE PAYLOAD
            pcall(function()
                CobaltEvent:FireServer("Section5/WireBoxComplete", targetBox)
            end)
            task.wait(0.15) -- Small network interval spacing

            -- B. COBALT SCREEN UI RELEASE INJECTION (Clears the interface blockage!)
            pcall(function()
                CobaltEvent:FireServer("Section5/WireBoxRelease", targetBox)
            end)
            notify("Screen Cleared", targetBox.Name .. " closed and bypassed cleanly!", 1)
            
            -- Blacklist current unique instance pointer address to safely transition to the next block
            _G.ClearedWireBoxes[targetBox] = true
            task.wait(0.4) -- Cooldown threshold spacing to shield package drop rates
        end
    end

    -- =========================================================================
    -- STEP 3: RETURN TO MAIN SWITCH AND FIRE FOR THE LAST TIME
    -- =========================================================================
    if _G.Section5Active and powerSwitchModel then
        notify("Step 3/3", "Returning to Main Switch for last activation...", 2)
        local switchCFrame = CFrame.new(-2037.31445, -353.530029, 4641.99805)
        ...
        if switchPrompt then
            safeFirePrompt(switchPrompt)
            notify("Success", "Main Switch finalized! Section 5 complete.", 3)
        else
            notify("Warning", "Final Switch prompt not found!", 3)
        end
    end
end)
