...
createToggle("Section 3", "Mizuno Box & Name ESP", "Highlights Mizuno and enables the 25-stud escape radar", function(state)
    ...
    -- Function para sa Pangalan (Name Tag) sa itaas ng ulo ni Mizuno
    local function applyNameTag(part)
        ...
        label.BackgroundTransparency = 1
        label.Text = "Mizuno"
        label.TextColor3 = Color3.fromRGB(0, 255, 255) -- Neon Cyan Outline para sa Section 3
        label.TextSize = 20
        label.Font = Enum.Font.SourceSansBold
        label.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
        label.TextStrokeTransparency = 0
    ...
    local function scanSection3Parts(object)
        ...
        -- Exact Path Matching para kay Mizuno sa Section 3
    ...
    if state then
        ...
        if mizunoFolder then
            ...
            if hrp then apply3DBox(hrp) end
        end

        -- 2. Makinig sa buong Workspace gamit ang DescendantAdded para sa live tracking (invisible mechanic bypass)
        ...
        if not _G.Section3RadarActive then
            ...
            task.spawn(function()
                -- Iyong eksaktong safe spot para sa Section 3
                local section3SafeZone = CFrame.new(-3584.720, 8.000, 247.522)
                local safetyDistanceThreshold = 25 -- Selyadong 25 studs batay sa iyong timpla

                while _G.Section3RadarActive do
                    task.wait(0.15) -- Mabilis na calculation tick rate
                    
                    local character = player.Character
                    local currentHRP = character and character:FindFirstChild("HumanoidRootPart")
                    
                    -- Hanapin si Mizuno gamit ang eksaktong path mo
                    ...
                    if threatPart and currentHRP then
                        ...
                        if distance <= safetyDistanceThreshold then
                            ...
                            task.wait(2.5) -- Loop freeze protection cooldown
                        end
                    end
                end
            end)
        ...
        -- ===== [TOGGLE OFF] =====
        -- 1. Patayin ang background listener connection
        if section3Connection then
            section3Connection:Disconnect()
            section3Connection = nil
        end
        
        -- 2. Patayin ang SafeZone Radar loop para sa Section 3
        _G.Section3RadarActive = false
        
        -- 3. Linisin ang screen at burahin ang lahat ng SelectionBox at Name Tags ni Mizuno
        if espContainer then
            espContainer:ClearAllChildren()
        end
...
createCustomButton("Section 3", "Restore Power Auto-Farm", "Teleport, interact, and bypass all circuit minigames", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    ...
    -- Siguraduhing umiiral ang folder ng mga circuit bago mag-umpisa
    ...
    if not circuitsFolder then
        notify("Error", "Hindi mahanap ang Circuits folder sa Section 3!", 4)
        return
    end

    -- Ang iyong 6 na eksaktong CFrame listahan para sa mga circuits
    ...
    -- Helper function para hanapin kung aling ProximityPrompt ang pinakamalapit sa player pagkalapag
    local function getClosestCircuitPrompt()
        local closestPrompt = nil
        local shortestDistance = math.huge
        local currentTargetModel = nil

        -- I-scan ang lahat ng laman sa folder (kahit ano pang index o pangalan nito)
        for _, obj in ipairs(circuitsFolder:GetChildren()) do
            ...
            if prompt and promptPart then
                ...
                if distance < shortestDistance then
                    shortestDistance = distance
                    closestPrompt = prompt
                    currentTargetModel = obj -- I-save ang model reference para sa Remote argument
                end
            end
        end
        return closestPrompt, currentTargetModel
    end

    -- Hanapin ang Remote Event para sa minigame bypass sa ReplicatedStorage
    local reliableRemote = ReplicatedStorage:FindFirstChild("MainRemote") 
        or ReplicatedStorage:FindFirstChild("RemoteEvent")
        or ReplicatedStorage:FindFirstChild("Section3/CircuitRoundComplete") -- Fallback check kung literal ang pangalan

    notify("Farm Started", "Sisimulan na ang pag-restore ng kuryente sa 6 na circuits...", 3)

    -- Loop sa iyong 6 na safe CFrame positions
    for index, targetCFrame in ipairs(circuitPositions) do
        notify("Progress Check", "Teleporting to Circuit [" .. index .. "/6]...", 1.5)
        
        -- Bypass vertical state lock bago lumipat
        pcall(function()
            humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
        end)
        task.wait(0.02)
        ...
        humanoidRootPart.CFrame = targetCFrame
        task.wait(0.5) -- Sapat na delay para mag-sync ang posisyon at mag-render ang model

        -- Hanapin ang prompt na pinakamalapit sa spot na ito
        local prompt, circuitModel = getClosestCircuitPrompt()

        if prompt and circuitModel then
            -- 1. I-fire ang ProximityPrompt para simulan ang circuit minigame interaction
            safeFirePrompt(prompt)
            task.wait(0.3) -- Sandaling wait para irehistro ng server na binuksan ang UI

            -- 2. I-fire ang Remote Event Bypass para tapusin agad ang minigame (Instant Win)
            if reliableRemote then
                -- Ipinapasa ang Arg1 at ang dynamic na pangalan ng circuit object (gaya ng CircuitPillar) bilang Arg2
                reliableRemote:FireServer("Section3/CircuitRoundComplete", circuitModel.Name)
                notify("Circuit Bypassed", "Matagumpay na natapos ang Circuit #" .. index .. "!", 1.5)
            else
                notify("Bypass Warning", "Hindi mahanap ang Remote instance. Manu-manong laruin muna!", 3)
            end
            
            task.wait(1.5) -- Ligtas na pahinga bago lumipat sa susunod na circuit (iwas anti-cheat kick)
        else
            notify("Warning", "Walang nahanap na circuit prompt sa Spot #" .. index, 2)
            task.wait(0.5)
        end
    end

    notify("Success", "Tapos na ang pag-restore sa lahat ng 6 na Circuits!", 4)
end)
