...
-- ====================================================================
-- AUTO C4 BOMB PART 2 (ANTI-FREEZE LOAD & SHADOW VARIABLE PROTECTED)
-- ====================================================================
createCustomButton("Section 4", "Auto C4 Bomb Part 2", "Tweens to trigger zone, then glides and plants all 6 C4 bombs", function()
    local Workspace = game:GetService("Workspace")
    local RunService = game:GetService("RunService")
    local StarterGui = game:GetService("StarterGui")
    
    -- Dynamic variable resolution sa mismong sandali ng pagpindot (Iwas infinite yield loading freeze)
    ...
    -- Safety check kung sakaling walang character instance loaded ang player
    if not currentHRP or not currentHumanoid then
        notify("Execution Error", "Character not fully loaded! Please reset or respawn first.", 3)
        return
    end
    ...
    local function fastTweenTo(targetCFrame, speed)
        ...
        local tweenInfo = TweenInfo.new(duration, Enum.EasingStyle.Linear)
        local tween = game:GetService("TweenService"):Create(currentHRP, tweenInfo, {CFrame = targetCFrame})
    ...
    local function safeFirePromptWithLock(prompt, targetCFrame)
        ...
        local lockConnection
        lockConnection = RunService.Heartbeat:Connect(function()
            if positionLockActive and currentHRP then
                currentHRP.CFrame = targetCFrame
                currentHRP.Velocity = Vector3.new(0, 0, 0) -- Freezes any back-push momentum [INDEX]
            end
        end)
