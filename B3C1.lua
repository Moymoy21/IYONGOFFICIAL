...
--	medkit
--	CFrame.new(212.713, 33.755, 342.602)
--	Workspace.Section2.Items.Bandage.Handle.ProximityPrompt
	
--	help Hideo
--	CFrame.new(165.188, 7.533, 336.290)
--	Workspace.Section2.NPC.Hideo3.HumanoidRootPart.ProximityPrompt
[TARGET REMOTE] (FireServer) -> Reliable
   Arg[1]: Section2/HideoMinigameStarted
...
createCustomButton("Section 1", "Hideo Interact", "Teleport to Hideo and Interact", function()
    ...
    -- 1. I-teleport ang player sa iyong safe CFrame
    humanoidRootPart.CFrame = CFrame.new(107.692, 7.064, 269.127)
    task.wait(0.5) -- Tinaasan sa 0.5 para siguradong naka-render at nakatapak ang character mo

    -- 2. Hanapin ang ProximityPrompt gamit ang safe dynamic check sa folder
    ...
    if prompt then
        -- 3. I-bypass ang mga posibleng pumipigil na restriction ng laro
        local originalRequiresLineOfSight = prompt.RequiresLineOfSight
        local originalMaxDistance = prompt.MaxActivationDistance
        
        prompt.RequiresLineOfSight = false
        prompt.MaxActivationDistance = 50 -- Siguradong abot kahit may kaunting harang o layo
        task.wait(0.1)

        -- 4. I-fire ang prompt at mag-notify sa gilid
        fireproximityprompt(prompt)
        notify("Success", "Naka-interact na kay Hideo!", 2)

        -- 5. Ibalik sa dati ang properties (para sa kaligtasan ng account)
        prompt.RequiresLineOfSight = originalRequiresLineOfSight
        prompt.MaxActivationDistance = originalMaxDistance
    else
...
createCustomButton("Section 1", "Unlock Gate & School Door", "Get Key and Unlock The Gate and School Door", function()
    ...
    -- Helper function para i-bypass ang restrictions ng prompt bago ito i-fire
    local function safeFirePrompt(prompt)
        ...
        prompt.RequiresLineOfSight = false
        prompt.MaxActivationDistance = 50 -- Tinaasan para siguradong abot kahit may harang
        task.wait(0.1)
        
        fireproximityprompt(prompt)
        
        -- Ibalik sa dati ang properties pagkatapos i-fire (iwas detection)
        prompt.RequiresLineOfSight = originalRequiresLineOfSight
        prompt.MaxActivationDistance = originalMaxDistance
        return true
    end
    ...
    -- =========================================================================
    -- STAGE 1: Kuhanin ang BoltCutter (Key)
    -- =========================================================================
    notify("Stage 1/3", "Papunta na sa BoltCutter...", 2)
    humanoidRootPart.CFrame = CFrame.new(202.509, 7.060, 109.554)
    task.wait(0.5) -- Binigyan ng oras para mag-sync ang posisyon sa server
    ...
    task.wait(1) --穩 (Stable) delay bago lumipat sa susunod na area

    -- =========================================================================
    -- STAGE 2: Buksan ang Gate Lock (Apat na beses i-fire)
    -- =========================================================================
    ...
    if gatePrompt then
        for i = 1, 4 do
            ...
            task.wait(0.3) -- Bahagyang tinaasan ang pagitan ng click para hindi ma-ignore ng server
        end
...
-- Global storage para sa connection para madaling ma-disconnect mamaya
local akariConnection = nil

createToggle("Section 2", "Akari Box & Name ESP", "Creates a 3D Box and Name Tag over Akari that bypasses invisibility", function(state)
    -- Kumuha o gumawa ng lalagyan sa CoreGui para sa mga visual elements
    local espContainer = CoreGui:FindFirstChild("IOHUB_AkariBoxContainer")
    if not espContainer then
        espContainer = Instance.new("Folder")
        espContainer.Name = "IOHUB_AkariBoxContainer"
    ...
    -- Function para lagyan ng 3D Adornment Box ang partikular na target part
    local function apply3DBox(part, customSize)
        if not part or not part:IsA("BasePart") then return end
        ...
        box.Name = existingBoxName
        box.Size = customSize or part.Size or Vector3.new(4, 6, 4)
    ...
    -- Function para hanapin at suriin ang mga target parts ni Akari sa Section 2
    local function scanAkariParts(object)
        ...
        if object.Parent.Name == "Akari" then
            if object.Name == "Hitbox" then
                apply3DBox(object)
                if not espContainer:FindFirstChild("Tag_HumanoidRootPart") then
                    applyNameTag(object)
                end
            elseif object.Name == "HumanoidRootPart" then
                apply3DBox(object, Vector3.new(3, 5, 3))
            end
        end
    end

    -- =========================================================================
    -- STATE EVALUATION (ON / OFF TRIPPERS)
    -- =========================================================================
    if state then
        -- ===== [TOGGLE ON] =====
        -- 1. I-check agad kung naka-render na ba si Akari sa Workspace sa simula
        local monsterFolder = Workspace:FindFirstChild("Section2") 
            and Workspace.Section2:FindFirstChild("Monster") 
            and Workspace.Section2.Monster:FindFirstChild("Akari")

        if monsterFolder then
            local targetHitbox = monsterFolder:FindFirstChild("Hitbox")
            local targetHRP = monsterFolder:FindFirstChild("HumanoidRootPart")
            
            if targetHitbox then apply3DBox(targetHitbox) end
            if targetHRP then apply3DBox(targetHRP, Vector3.new(3, 5, 3)) end
        end

        -- 2. Makinig sa buong Workspace gamit ang DescendantAdded para sa pag-invisible/respawn
        ...
        StarterGui:SetCore("SendNotification", {
            ...
            Text = "📦 Akari 3D Box + Name Tag ESP: ACTIVE",
            Duration = 3
        })
    else
        -- ===== [TOGGLE OFF] =====
        -- 1. Putulin ang background listener para iwas memory leak
        if akariConnection then
            akariConnection:Disconnect()
            akariConnection = nil
        end
        
        -- 2. Burahin ang lahat ng nakuhang visual elements (kahon at pangalan)
        ...
        StarterGui:SetCore("SendNotification", {
            Title = "IOHUB ESP Status",
            Text = "⚪ Akari 3D Box + Name Tag ESP: DISABLED",
            Duration = 2
        })
    end
end)
...
--school safe zone CFrame.new(174.887, 9.931, 368.043)

--equip gun
--workspace.GunSystem.PistolDummy.Gun
...
local safetyDistanceThreshold = 45 -- Range sa studs; kapag pumasok si Akari sa distansyang ito, lilikas ka agad
...
-- =========================================================================
-- ANTI-GRAVITY ENGINE (Pinapanatili kang lutang at hindi natutumba habang bumaril)
-- =========================================================================
if not _G.AntiGravityActive then
    _G.AntiGravityActive = true
    RunService.Heartbeat:Connect(function()
        if _G.AntiGravityActive and character and humanoid and humanoidRootPart then
            -- Pinipilit ang Humanoid na manatili sa 'Physics' state para walang ragdoll o fall animation
            humanoid:ChangeState(Enum.HumanoidStateType.Physics)
            -- Kinakansela ang pababang velocity ng gravity para hindi ka mahulog sa map habang bumaril
...
-- =========================================================================
-- ANTI-AKARI RADAR DETECTOR (Awtomatikong Emergency Teleport sa Safe Zone)
-- =========================================================================
if not _G.AkariRadarActive then
    _G.AkariRadarActive = true
    task.spawn(function()
        while _G.AkariRadarActive do
            task.wait(0.2) -- Bilis ng pag-scan sa paligid
            
            -- Hanapin ang kasalukuyang pwesto ng boss na si Akari
            local akariModel = Workspace:FindFirstChild("Section2") 
                and Workspace.Section2:FindFirstChild("Monster") 
                and Workspace.Section2.Monster:FindFirstChild("Akari")
                
            local akariHRP = akariModel and (akariModel:FindFirstChild("HumanoidRootPart") or akariModel:FindFirstChild("Hitbox"))
            
            if akariHRP and character and humanoidRootPart then
                local currentDistance = (humanoidRootPart.Position - akariHRP.Position).Magnitude
                
                -- Kapag pumasok si Akari sa iyong danger zone range
                if currentDistance <= safetyDistanceThreshold then
                    notify("⚠️ DANGER DETECTED", "Masyadong malapit si Akari! Emergency Teleport sa Safe Zone!", 3)
                    humanoidRootPart.CFrame = safeZoneCFrame
                    task.wait(2) -- Cooldown bago payagang mag-teleport ulit para iwas loop trigger
                end
            end
        end
    end)
...
createCustomButton("Section 2", "Cycle Akari Spiders", "Teleports through spider spots 1-18 sequentially", function()
    -- I-refresh ang local instances para sa panibagong character respawn checks
    ...
    if targetCFrame then
        ...
        -- Sapilitang i-break ang vertical state at i-teleport ka sa spot
        pcall(function()
        ...
        -- I-advance ang counter para sa susunod na pindot mo
        if _G.SpiderCycleIndex >= #spiderLocations then
            _G.SpiderCycleIndex = 1 -- Ibalik sa una kapag natapos ang pang-18
            notify("Cycle Reset", "Natapos ang lahat ng spots! Babalik sa Spider #1 sa susunod na pindot.", 2)
        else
...
createCustomButton("Section 2", "Emergency SafeZone", "Teleport to SafeZone and Stop Farming", function()
    ...
    -- =========================================================================
    -- EMERGENCY STOP & RESET SYSTEM
    -- =========================================================================
    -- Patayin ang Anti-Gravity/Physics freeze bypass para makalakad ka nang normal
    _G.AntiGravityActive = false 
    
    -- Ibalik sa Spider #1 ang index para malinis ang susunod na simula ng cycle
    _G.SpiderCycleIndex = 1 
    
    -- Ibalik ang Humanoid state sa default Standing state
    pcall(function()
