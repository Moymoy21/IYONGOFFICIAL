...
-- Anti-Push Engine para sa Part 2
local function safeFirePromptWithLock(prompt, targetCFrame)
    if not prompt then return false end
    ...
    lockConnection = RunService.Heartbeat:Connect(function()
        if positionLockActive and humanoidRootPart then
            humanoidRootPart.CFrame = targetCFrame
            humanoidRootPart.Velocity = Vector3.new(0, 0, 0) -- Putulin ang tulak ng bato/physics
        end
    end)
    
    fireproximityprompt(prompt)
    task.wait(0.6) -- Selyadong frame verification time
...
-- 1. INITIAL AREA TRANSITION TELEPORT
notify("Transition", "Moving to initial trigger zone...", 2)
local transitionZone = CFrame.new(-4146.615, 106.991, 2298.599)
pcall(function() humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end)
task.wait(0.01)
humanoidRootPart.CFrame = transitionZone
task.wait(0.6) -- Wait buffer para mag-render ang bagong kwarto [INDEX]

-- Listahan ng iyong 6 na C4 coordinates para sa Part 2 [INDEX]
local bombLocations = {
...
for index, targetCFrame in ipairs(bombLocations) do
    notify("Progress", "Moving to C4 Bomb [" .. index .. "/6]...", 1)
    
    pcall(function() humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end)
    task.wait(0.01)
    humanoidRootPart.CFrame = targetCFrame
    task.wait(0.5) -- Sapat na delay para mag-sync ang posisyon sa bagong spot
