...
createCustomButton("Section 4", "Cleanse Room Farm", "Automates sequential 4x valve turner stages and teleports to contain", function()
    local Players = game:GetService("Players")
    local Workspace = game:GetService("Workspace")
    local StarterGui = game:GetService("StarterGui")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    ...
    -- =========================================================================
    -- PHASE 1: SEQUENTIAL 3-STAGE VALVE TURNER CLICKS (4X PER PROMPT FRAME)
    -- =========================================================================
    notify("Phase 1/2", "Starting Valve progressive click engine...", 2)
    task.wait(0.2)

    local _valvesFolder = cleanseFolder:FindFirstChild("Valves")
    if _valvesFolder then
        local valveModelsList = _valvesFolder:GetChildren()
        notify("Valves Scan", "Found " .. tostring(#valveModelsList) .. " Valve models to clear.", 2.5)

        for _, valveModel in ipairs(valveModelsList) do
            if valveModel:IsA("Model") then
                notify("Valve Locked", "Processing: " .. valveModel.Name, 1.5)
                
                pcall(function() humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end)
                task.wait(0.01)
                
                -- Teleport to current valve's exact model center coordinates
                humanoidRootPart.CFrame = valveModel:GetPivot()
                task.wait(0.6) -- Mobile loading positioning sync buffer

                -- Mag-loop ng 3 beses para sa 3 magkakahiwalay na Turner stages ng iisang Valve model
                for stage = 1, 3 do
                    local activePrompt = nil
                    
                    -- Dynamic lookup scanner para hanapin kung aling prompt ang binuhay at "Enabled" sa stage na ito
                    for _, descendant in ipairs(valveModel:GetDescendants()) do
                        if descendant:IsA("ProximityPrompt") and descendant.Enabled then
                            activePrompt = descendant
                            break
                        end
                    end

                    if activePrompt then
                        notify("Valve Processing", valveModel.Name .. " - Stage (" .. stage .. "/3) Active", 1.5)
                        
                        -- SELYADONG 4X CLICK ENGINE LOOP: Pipindutin ng selyadong apat na beses ang bagong litaw na turner
                        for clickCount = 1, 4 do
                            notify("Turning Valve", "Firing current prompt: (" .. clickCount .. "/4)", 0.8)
                            safeFirePrompt(activePrompt)
                            task.wait(0.25) -- Safe fire buffer spacing to sync with server ticks
                        end
                        
                        -- NAPAKAHALAGA: 0.5s loading buffer window para payagan ang server na i-load at i-spawn ang kasunod na turner model stage
                        task.wait(0.5) 
                    else
                        notify("Stage Link Skip", "No active prompt found for Stage " .. stage .. ". Moving forward...", 1.5)
                    end
                end
                
                notify("Valve Success", valveModel.Name .. " has been fully pressurized!", 1.5)
                task.wait(0.4)
            end
        end
    end
    ...
    -- =========================================================================
    -- PHASE 2: AUTOMATIC CONTROL TERMINALS SYSTEM (SHAPE TERMINALS)
    -- =========================================================================
    notify("Phase 2/2", "All valves completed! Initializing Terminals scan...", 2)
    task.wait(0.2)
    ...
    -- =========================================================================
    -- FINAL CONTAINER DESTINATION TELEPORTATION (BAGONG DAGDAG)
    -- =========================================================================
    notify("Quest Finished", "All objectives cleared! Teleporting to Contain location...", 2.5)
    task.wait(0.5)
